...
/**
 * Class Scoreboard
 * Guarda un HTML con el marcador final
...
public class Scoreboard {
  ...
  private void saveStyle(Document html, Element root) {
    ...
    style.appendChild(html.createTextNode("h1{"
        + "    text-align: center;"
        + "  }"
        + "  table {"
        + "  width: 100%;"
  ...
  private void saveTbody(Document html,
      Element table) {
    ...
    for (int i = 0; i < this.rounds; i++) {
      ...
      for (int j = 0; j < this.numPlayers; j++) {
        if(i < this.players.get(j).getRoundPoints().size()) {
          ...
          tdRound.appendChild(html.createTextNode(String.valueOf(this.players.get(j).getRoundPoints().get(i))));
  ...
  private void saveTfoot(Document html, Element table) {
    ...
    for (int i = 0; i < this.numPlayers; i++) {
      ...
      thTotalPoint.appendChild(html.createTextNode(String.valueOf(this.players.get(i).getTotalPoints())));
